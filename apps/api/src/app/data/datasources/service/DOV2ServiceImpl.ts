...
export class DOV2ServiceImpl implements DOService {
  ...
  createDomain(name: string, ip: string): Promise<DODomainEntity> {
    return this._post("/domains", { name, ip_address: ip }).then((response) => {
  ...
  createSubdomain(
    ...
    return this._post(`/domains/${domain}/records`, subdomainOptions).then(
      (response) => {
        if (response.status !== 201) {
          throw new Error(
            `Error creating the subdomain '${name}' for domain '${domain}' in DigitalOcean`
          );
        }
  ...
  deleteDomain(name: string): Promise<void> {
    return this._delete(`/domains/${name}`).then((response) => {
      if (response.status !== 204) {
        throw new Error(
          `Error deleting the domain '${name}' from DigitalOcean`
        );
      }
    });
  }
  ...
  deleteSubdomain(domainRecordID: string, domain: string): Promise<void> {
    return this._delete(`/domains/${domain}/records/${domainRecordID}`).then(
      (response) => {
        if (response.status !== 204) {
          throw new Error(
            `Error deleting the A record with id '${domainRecordID}' of domain '${domain}' from DigitalOcean.`
          );
        }
      }
    );
  ...
  refreshDomainsAndSubdomains(): Promise<DODomainEntity[]> {
    return this._get("/domains").then((response) =>
      (response.data as GetDomainsResponse).domains.map((d) => ({
        name: d.name,
        ttl: d.ttl,
